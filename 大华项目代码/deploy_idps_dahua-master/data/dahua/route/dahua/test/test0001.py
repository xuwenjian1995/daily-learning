...
host = 'extract_admin_html:80/api/'
user_info = dict(token='', username='admin', password='abcd@123!')
def login():
    """登录
    """
...
def send_request_check(file_path,doc_form=1):
    ...
    response = request_with_jwt(
        url=host + 'review',
        data = {"data": json.dumps({"check_point_id_list": [47], "tag_type_id": "2425","doc_form":doc_form}), "async_task": False},
        files =fileList,
        method='POST')
    '''
    {
...
# send_request_check("1.pdf",doc_form=1)
send_request_check_info(21)
