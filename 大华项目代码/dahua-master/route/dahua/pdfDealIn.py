...
from tools.utils.convertPDF import convert_pdf_to_png,shot_of_png,qrcode_parser,get_first_image_from_pdf
...
def pdfdeal(file_uuid):
    doc_code = False
    # 将pdf转换成图片
    if get_first_image_from_pdf(file_uuid):  # file_uuid_page.png
        # 截图 正文
        if shot_of_png(file_uuid, location_middle, type="middle"):
            ...
            if message:
                ...
                pat_middle = u"审编号为[\_\.,\s]{0,2}(\d[\d\s\-]+\d)"
